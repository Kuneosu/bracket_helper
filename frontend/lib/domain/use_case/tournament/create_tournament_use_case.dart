...
class CreateTournamentParams {
  ...
  const CreateTournamentParams({required this.title, required this.date});

  @override
  String toString() => 'CreateTournamentParams(title: $title, date: $date)';
}
...
class CreateTournamentUseCase {
  ...
  Future<Result<int>> execute(CreateTournamentParams params) async {
    try {
      ...
      // 디버그 로그
      print('CreateTournamentUseCase: 토너먼트 생성 시도 - ${params.title}');
      ...
      // 디버그 로그
      print('CreateTournamentUseCase: 토너먼트 생성 성공 - ID: $tournamentId');

      // tournamentId가 Result<int> 타입이면 그대로 반환, int 타입이면 Result로 래핑
      return tournamentId;
    } catch (e) {
      // 디버그 로그
      print('CreateTournamentUseCase: 토너먼트 생성 실패 - $e');

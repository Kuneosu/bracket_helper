...
class CreateTeamParams {
  ...
  CreateTeamParams({required this.player1Id, this.player2Id});
}
...
class CreateTeamUseCase {
  ...
  Future<Result<int>> execute(TeamModel params) async {
    // 디버깅용 로그
    debugPrint(
      'CreateTeamUseCase: 팀 생성 요청 - 선수1: ${params.p1.id}, 선수2: ${params.p2?.id}',
    );

    // 선수1 존재 여부 확인
    final player1Result = await _playerRepository.getPlayer(params.p1.id);
    ...
    if (player1Result.value == null) {
      debugPrint('CreateTeamUseCase: 선수1이 존재하지 않음 - ID: ${params.p1.id}');
      return Result.failure(
        TeamError(message: '선수1(ID: ${params.p1.id})이 존재하지 않습니다.'),
      );
    }

    // 선수2가 지정된 경우, 존재 여부 확인
    if (params.p2 != null) {
      final player2Result = await _playerRepository.getPlayer(params.p2!.id);
      ...
      if (player2Result.value == null) {
        debugPrint('CreateTeamUseCase: 선수2가 존재하지 않음 - ID: ${params.p2!.id}');
        return Result.failure(
          TeamError(message: '선수2(ID: ${params.p2!.id})가 존재하지 않습니다.'),
        );
      }

      // 동일한 선수로 팀을 만들려는 경우 오류
      if (params.p1.id == params.p2!.id) {
        debugPrint('CreateTeamUseCase: 동일한 선수로 팀 생성 시도');
        return Result.failure(TeamError(message: '동일한 선수로 팀을 구성할 수 없습니다.'));
      }
    }
    ...
    try {
      ...
      final team = TeamsCompanion.insert(
        player1Id: params.p1.id,
        player2Id:
            params.p2 != null ? Value(params.p2!.id) : const Value.absent(),
      );
      ...
      return Result.failure(TeamError(message: '팀 생성 중 오류가 발생했습니다.', cause: e));
    }
  }
}

...
class PartnerTournamentInfoScreen extends StatefulWidget {
  final TournamentModel tournament;
  final Function(CreatePartnerTournamentAction) onAction;
  const PartnerTournamentInfoScreen({
    super.key,
  ...
  @override
  State<PartnerTournamentInfoScreen> createState() =>
      _PartnerTournamentInfoScreenState();
}

class _PartnerTournamentInfoScreenState
    extends State<PartnerTournamentInfoScreen>
  ...
  void _onFocusChange() {
    if (!_titleFocusNode.hasFocus) {
      ...
      widget.onAction(
        CreatePartnerTournamentAction.onTitleChanged(_titleController.text),
      );
    }
  }

  @override
  void didUpdateWidget(PartnerTournamentInfoScreen oldWidget) {
    super.didUpdateWidget(oldWidget);
    debugPrint('PartnerTournamentInfoScreen - didUpdateWidget 호출');

    if (oldWidget.tournament.title != widget.tournament.title) {
      _titleController.text = widget.tournament.title;
      debugPrint(
        'PartnerTournamentInfoScreen - 타이틀 변경됨: ${widget.tournament.title}',
      );
    }

    if (oldWidget.tournament.date != widget.tournament.date) {
      debugPrint(
        'PartnerTournamentInfoScreen - 날짜 변경됨: ${widget.tournament.date}',
      );
    }
  }
  ...
  void dispose() {
    ...
    debugPrint('PartnerTournamentInfoScreen - dispose 호출');
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    debugPrint(
      'PartnerTournamentInfoScreen - build 호출: 날짜 ${widget.tournament.date}',
    );
      ...
      content: Column(
        ...
        children: [
          TextField(
            ...
            onSubmitted: (value) {
              widget.onAction(
                CreatePartnerTournamentAction.onTitleChanged(value),
              );
            },
          ),
    ...
    return SectionCardWidget(
      ...
      content: DefaultDatePicker(
        ...
        onDateSelected: (date) {
          ...
          widget.onAction(CreatePartnerTournamentAction.onDateChanged(date));
        },
      ),
    );
  }
          ...
          ScoreInputWidget(
            ...
            onChanged:
                (value) => widget.onAction(
                  CreatePartnerTournamentAction.onScoreChanged(value, '승'),
                ),
          ),
          ScoreInputWidget(
            ...
            onChanged:
                (value) => widget.onAction(
                  CreatePartnerTournamentAction.onScoreChanged(value, '무'),
                ),
          ),
          ScoreInputWidget(
            ...
            onChanged:
                (value) => widget.onAction(
                  CreatePartnerTournamentAction.onScoreChanged(value, '패'),
                ),
          ),
        ],
      ),
                  ...
                  child: GestureDetector(
                    onTap: () {
                      if (!widget.tournament.isDoubles) {
                        widget.onAction(
                          CreatePartnerTournamentAction.onIsDoublesChanged(
                            true,
                          ),
                        );
                      }
      ...
      onPrevious: () async {
        ...
        if (shouldExit) {
          if (mounted && ctx.mounted) {
            ...
            widget.onAction(CreatePartnerTournamentAction.onDiscard());
          }
        }
      },
      onNext: () {
          ...
          (context) => AlertDialog(
            ...
            actions: [
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  widget.onAction(CreatePartnerTournamentAction.resetState());
